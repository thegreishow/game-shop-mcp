...
`npm run playwright:local` is an explicit one-command bootstrap. In normal local autonomous jobs it should not be necessary because the browser QA path calls the same ensure routine automatically.
