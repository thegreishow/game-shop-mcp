import { ensureLocalBrowserAutonomy, localBrowserEndpoint } from "./local-browser-autonomy.js";
...
function playwrightUrl(){const url=localBrowserEndpoint();if(!url)throw new Error("No Playwright MCP endpoint is configured for this runtime.");return url;}
...
export async function executeBrowserQa(input:{url:string;provider?:BrowserQaProvider;interactions?:string[]}){const provider=configuredProvider(input.provider);if(provider==="playwright-mcp"){const local=await ensureLocalBrowserAutonomy({startIfNeeded:true});if(!local.ready)throw new Error(local.reason);}const endpoint=provider==="browserbase"?browserbaseUrl():playwrightUrl();const init=await mcpInitialize({url:endpoint,timeoutMs:15000});const listed=await mcpListTools({url:endpoint,sessionId:init.sessionId,timeoutMs:15000});let sessionId=listed.sessionId;const names=new Set(listed.tools.map(t=>t.name));const evidence:BrowserEvidence={provider,url:input.url,sessionId,tools:[...names],steps:[],console:[],network:[],screenshots:[],errors:[]};
...
export function browserAdapterInfo(){const localEndpoint=localBrowserEndpoint();return{browserbase:{configured:Boolean(process.env.BROWSERBASE_API_KEY),endpoint:"https://mcp.browserbase.com/mcp",mode:"hosted-stagehand-mcp"},playwright:{configured:Boolean(localEndpoint),endpoint:localEndpoint,mode:"streamable-http-mcp",recommendedCommand:"npm run playwright:mcp",automaticLocalStart:true}};}
