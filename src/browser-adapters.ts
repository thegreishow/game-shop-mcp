...
function interactionCode(action:string){
 const trimmed=action.trim();
 if(trimmed.startsWith("click:")){const selector=trimmed.slice(6).trim();return`async (page)=>{const target=page.locator(${JSON.stringify(selector)}).first();await target.waitFor({state:'visible',timeout:5000});await target.click({timeout:5000});return{action:'click',selector:${JSON.stringify(selector)},url:page.url()};}`;}
 if(trimmed.startsWith("press:")){const key=trimmed.slice(6).trim();return`async (page)=>{await page.keyboard.press(${JSON.stringify(key)});return{action:'press',key:${JSON.stringify(key)},url:page.url()};}`;}
 if(trimmed.startsWith("wait:")){const ms=Math.max(0,Math.min(5000,Number(trimmed.slice(5))||0));return`async (page)=>{await page.waitForTimeout(${ms});return{action:'wait',ms:${ms},url:page.url()};}`;}
 if(trimmed.startsWith("fill:")){const body=trimmed.slice(5);const split=body.indexOf("=");if(split>0){const selector=body.slice(0,split).trim(),value=body.slice(split+1);return`async (page)=>{const target=page.locator(${JSON.stringify(selector)}).first();await target.fill(${JSON.stringify(value)},{timeout:5000});return{action:'fill',selector:${JSON.stringify(selector)},url:page.url()};}`;}}
 return`async (page)=>({action:'intent-only',intent:${JSON.stringify(trimmed.slice(0,300))},url:page.url(),title:await page.title()})`;
}
export async function executeBrowserQa(input:{url:string;provider?:BrowserQaProvider;interactions?:string[];assertions?:BrowserAssertion[]}){const provider=configuredProvider(input.provider);const endpoint=provider==="browserbase"?browserbaseUrl():playwrightUrl();const init=await mcpInitialize({url:endpoint,timeoutMs:15000});const listed=await mcpListTools({url:endpoint,sessionId:init.sessionId,timeoutMs:15000});let sessionId=listed.sessionId;const names=new Set(listed.tools.map(t=>t.name));const assertions=(input.assertions??[]).slice(0,20);const evidence:BrowserEvidence={provider,url:input.url,sessionId,tools:[...names],steps:[],console:[],network:[],screenshots:[],errors:[]};
 ...
 if(names.has("browser_run_code")){
  const code=`async (page) => {\nconst consoleErrors=[]; const failedRequests=[]; const badResponses=[];\npage.on('console',m=>{if(m.type()==='error') consoleErrors.push(m.text())});\npage.on('requestfailed',r=>failedRequests.push({url:r.url(),failure:r.failure()?.errorText||'failed'}));\npage.on('response',r=>{if(r.status()>=400) badResponses.push({url:r.url(),status:r.status()})});\nawait page.goto(${JSON.stringify(input.url)},{waitUntil:'domcontentloaded',timeout:30000});\nawait page.waitForTimeout(500);\nconst hard=[...consoleErrors.map(message=>({kind:'console',message})),...failedRequests.map(value=>({kind:'requestfailed',...value})),...badResponses.map(value=>({kind:'http',...value}))];\nif(hard.length) throw new Error('GAME_SHOP_BROWSER_JUDGE:'+JSON.stringify({hard}));\nreturn {passed:true,consoleErrors,failedRequests,badResponses,title:await page.title(),url:page.url()};\n}`;
  await invoke("browser_run_code",{code});
 }else await invoke("browser_navigate",{url:input.url});
 const snap=await invoke("browser_snapshot");if(snap)evidence.snapshot=snap;
 for(const action of (input.interactions??[]).slice(0,8))if(names.has("browser_run_code"))await invoke("browser_run_code",{code:interactionCode(action)});
 if(names.has("browser_run_code")){
  const finalCode=`async (page)=>{await page.waitForTimeout(250);const assertions=await page.evaluate(()=>{const failures=[];${assertionCode(assertions)}return{passed:failures.length===0,failures,title:document.title,url:location.href,readyState:document.readyState,bodyTextLength:(document.body?.innerText||'').length};});if(!assertions.passed)throw new Error('GAME_SHOP_BROWSER_JUDGE:'+JSON.stringify({hard:assertions.failures.map(message=>({kind:'assertion',message})),assertions}));return assertions;}`;
  const judged=await invoke("browser_run_code",{code:finalCode});if(judged)evidence.assertions=judged;
 }
