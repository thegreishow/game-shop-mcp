...
export function redirectAllowed(raw:string){try{const url=new URL(raw);const extra=(process.env.GAME_SHOP_OAUTH_REDIRECT_HOSTS||"").split(",").map(v=>v.trim()).filter(Boolean);if(url.protocol==="https:"){const allowed=["grok.com","x.ai","chatgpt.com","openai.com","game-shop-mcp.vercel.app","gameshop.thegreishow.com",...extra];return allowed.some(host=>url.hostname===host||url.hostname.endsWith(`.${host}`));}if(url.protocol==="http:"){return ["localhost","127.0.0.1","::1","[::1]"].includes(url.hostname);}return ["chatgpt:","openai:","com.openai.chatgpt:"].includes(url.protocol);}catch{return false;}}
