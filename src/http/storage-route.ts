...
import { z } from "zod";
import { artifactObjectStoreInfo, persistArtifactObject } from "../artifact-storage.js";
import { authorizeRequest, enforceRateLimit } from "../security.js";
...
export async function storageRoute(request:Request){const auth=authorizeRequest(request);if(!auth.ok)return new Response(JSON.stringify({error:auth.error}),{status:auth.status,headers:{"content-type":"application/json"}});const rate=enforceRateLimit(request);if(!rate.ok)return new Response(JSON.stringify({error:"Rate limit exceeded"}),{status:429,headers:{"content-type":"application/json"}});return handler(request)}
