...
export async function runQaSwarm(input:{projectId:string;ref?:string;url?:string;checks?:VerificationCheck[];interactions?:string[];autoPreview?:boolean;queuePlaywrightCi?:boolean}){
 ...
 const target=primary.url?String(primary.url):null;const lanes:Record<string,unknown>={primary};let errorCount=countErrors(primary);let ci:unknown=null;let remotePlaywright:unknown=null;
 if(target&&process.env.GAME_SHOP_PLAYWRIGHT_MCP_URL){try{remotePlaywright=await executeBrowserQa({url:target,provider:"playwright-mcp",interactions:input.interactions});lanes.playwrightRemote=remotePlaywright;const errors=(remotePlaywright as Awaited<ReturnType<typeof executeBrowserQa>>).errors.length;errorCount+=errors;}catch(error){errorCount++;lanes.playwrightRemote={status:"failed",error:error instanceof Error?error.message:String(error)};}}
 if(target&&(input.queuePlaywrightCi??true)&&process.env.GITHUB_TOKEN){try{ci=await dispatchPlaywrightCi({url:target,projectId:input.projectId,interactions:input.interactions});lanes.playwrightCi=ci;}catch(error){lanes.playwrightCi={status:"unavailable",error:error instanceof Error?error.message:String(error)};}}
 let gate:QaGate;if(!target)gate="blocked";else if(errorCount>0||primary.status==="failed")gate="red";else if(ci&&(ci as Record<string,unknown>).status==="queued")gate="provisional";else gate="green";
 const confidence=gate==="green"?1:gate==="provisional"?.8:gate==="blocked"?.25:0;
 return{schemaVersion:"1.0",projectId:input.projectId,ref:primary.ref,url:target,gate,confidence,errorCount,lanes,release:{allowed:gate==="green",reason:gate==="green"?"QA release gate is green.":gate==="provisional"?"Live QA passed; deterministic Playwright CI is still pending.":gate==="blocked"?"No preview/browser target is available.":"QA evidence contains errors."},info:qaSwarmInfo()};
}
