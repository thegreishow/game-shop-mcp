...
export function externalEngineStatus(){return{
  meshy:{configured:Boolean(process.env.MESHY_API_KEY),mode:"rest",paid:true,artifactCapture:true},
  fal:{configured:Boolean(process.env.FAL_KEY),mode:"queue-api",paid:true,artifactCapture:true},
  replicate:{configured:Boolean(process.env.REPLICATE_API_TOKEN),mode:"rest+mcp",paid:true,artifactCapture:true},
