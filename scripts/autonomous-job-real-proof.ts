...
async function main(){
  ...
  try{
    const job=await createAutonomousJob({
      goal:"Prove Dubai Legends works in Chromium using the canonical Game Shop browser-authoritative execution loop",
      projectId,
      autonomy:"plan",
      constraints:["read-only live proof","no repository mutation","Playwright must be the terminal judge"],
      maxRepairAttempts:3,
    });
    const judged=await judgeAutonomousJob({
      ...
      autoPreview:false,
      checks:[{path:"index.html",required:false}],
      interactions:["wait:1500","click:#start","wait:500"],
      assertions:[
        {kind:"title-includes",includes:"Dubai Legends"},
        {kind:"selector-visible",selector:"#scoreboard"},
        {kind:"selector-visible",selector:"#controls"},
        {kind:"selector-text",selector:"#round",includes:"QUALIFIER"},
        {kind:"selector-text",selector:"#hit",includes:"HIT"},
